...
export abstract class Key {
              ...
              .then((pingMsg: Message) => {
                ...
                chan.send(keyEmbed).then((m: Message) => {
                  ...
                  missingRoles.forEach((r) => {
                    ...
                    Utils.debug(
                      `adding reaction for \`${JSON.stringify(
                        r
                      )}\` using emoji ${emoji}`
                    );
                    m.react(emoji);
                  ...
                  m.react("🔒");
                  Utils.debug(
                    `adding reaction for \`"Delete"\` using emoji ❌`
                  );
                  m.react("❌");
                  Utils.debug(
                    `adding reaction for \`"Notify"\` using emoji 📢`
                  );
                  m.react("📢");
    ...
    roleCollector.on("collect", (reaction, user) => {
      ...
      if (reaction.emoji.name === "🔒") {
        if (isAdmin || isAuthor) {
          ...
          Utils.debug(
            `user \`${JSON.stringify(user)}\` not allowed to use ${
              reaction.emoji.name
            } reaction`
          );
        }
  ...
  private pingTeam(msg: Message, key: string): void {
    ...
    Utils.debug(
      `creating voice channel \`${JSON.stringify(voiceChannelName)}\``
    );
    ...
    msg.guild.channels
      ...
      .then((channel) => {
        ...
        let timeout = setTimeout(function () {
          Utils.debug(
            `deleting voice channel \`${JSON.stringify(
              voiceChannelName
        ...
        this.client.on("voiceStateUpdate", (oldState, newState) => {
          ...
          if (memberCount > 0) {
            Utils.debug(
              `activity found in voice channel \`${JSON.stringify(
                voiceChannelName
              )}\`, removing 'delete' timer`
            );
            clearTimeout(timeout);
          } else {
            Utils.debug(
              `no more members in voice channel \`${JSON.stringify(
                voiceChannelName
